...
async def stream_answer(query: str, history):
    ...
    # 2. LLM 설정 (창의성을 약간 허용하여 답변을 풍부하게 함)
    llm = ChatOllama(model=MODEL_NAME, temperature=0.15, streaming=True)
    ...
    for i, doc in enumerate(filtered_docs, 1):
        ...
        print(f"  {i:>2}순위: {fname:<40} (p.{doc.metadata.get('page', 0)+1})")
    print(f"{'='*85}")
    ...
    print(f"\n[STEP 2] 답변-문서 매칭 밀도 분석 (Threshold: 0.12)")
    print(f"{'-'*85}")
    ...
    for d in filtered_docs:
        ...
        status = "✅ PASS" if match_density >= 0.12 else "❌ DROP"
        print(f"{status:^8} | {f[:30]:<35} (p.{p}) | {match_density:.4f}")

        if match_density >= 0.12 and f"{f}_{p}" not in seen:
            sources.append({
                "file": f, "page": p, 
